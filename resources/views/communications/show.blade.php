...
<x-communications-layout :title="'#'.$channel->name">
    ...
    <div class="h-[calc(100vh-92px)] lg:h-[calc(100vh-108px)] min-h-[420px] flex border border-[var(--border-default)] rounded-[var(--radius-card)] overflow-hidden shadow-[var(--shadow-card)] bg-[var(--surface-card)]">
        ...
        <div class="flex-1 flex flex-col min-w-0" x-data="chatChannel(@js($chatConfig))">
            ...
            {{-- Header --}}
            <div class="h-14 border-b border-[var(--border-default)] flex items-center justify-between px-4 flex-shrink-0 bg-[var(--surface-card)] relative z-10">
                <div class="min-w-0 flex items-center gap-[10px]">
                    <div class="w-8 h-8 rounded-[8px] bg-[var(--color-primary-light)] flex items-center justify-center flex-shrink-0">
                    ...
                    <div class="min-w-0">
                        ...
                        @if($channel->description)
                            <p class="text-[12px] text-[var(--text-400)] truncate">{{ $channel->description }}</p>
                        @endif
                    </div>
                </div>
                @if($canManage)
                    <button type="button" @click="$dispatch('open-modal', 'miembros-canal')"
                            class="inline-flex items-center gap-[6px] px-3 py-1.5 rounded-[var(--radius-control)] text-[13px] text-[var(--text-500)] hover:bg-[var(--surface-muted)] hover:text-[var(--text-700)] flex-shrink-0">
            ...
            {{-- Mensajes: fondo blanco plano, a pedido explícito del usuario --}}
            <div class="flex-1 overflow-y-auto px-4 py-4 space-y-1 bg-[var(--surface-card)]" x-ref="scrollArea">
                <template x-if="messages.length === 0">
                    <div class="flex flex-col items-center justify-center h-full text-center px-6 -mt-8">
                        <div class="w-12 h-12 rounded-full bg-[var(--surface-card)] shadow-[var(--shadow-card)] flex items-center justify-center mb-3">
                            <x-lucide-message-circle class="w-5 h-5 text-[var(--color-primary)]" />
                        </div>
                        <p class="text-[13px] text-[var(--text-500)]">Sé el primero en escribir en este canal.</p>
                    </div>
                </template>

                <template x-for="(message, index) in messages" :key="message.id">
                    <div>
                        <template x-if="showDayDivider(messages, index)">
                            <div class="flex items-center justify-center my-4">
                                <span class="text-[11px] font-medium text-[var(--text-500)] bg-[var(--surface-card)] border border-[var(--border-default)] rounded-full px-3 py-1 shadow-[var(--shadow-card)]" x-text="dayLabel(message)"></span>
                            </div>
                        </template>
            ...
            <div class="border-t border-[var(--border-default)] p-3 flex-shrink-0 relative bg-[var(--surface-card)]">
                ...
                <div x-show="replyTo" x-cloak class="flex items-center gap-2 mb-2 pl-2.5 pr-3 py-1.5 bg-[var(--surface-subtle)] rounded-[var(--radius-control)] border-l-[3px] border-[var(--color-primary)]">
                    <div class="min-w-0 flex-1">
                        <p class="text-[12px] font-semibold text-[var(--color-primary)]">Respondiendo a <span x-text="replyToAuthor()"></span></p>
