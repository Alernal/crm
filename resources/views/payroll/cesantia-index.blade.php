...
<form method="GET" action="{{ route('cesantia-settlements.index', [], false) }}" class="flex flex-col sm:flex-row gap-3 mb-5">
    ...
    <a href="{{ route('cesantia-settlements.index', [], false) }}" class="h-10 flex items-center px-4 rounded-[var(--radius-control)] border border-[var(--border-default)] text-[var(--text-700)] text-[14px] font-medium hover:bg-[var(--surface-muted)]">
        Limpiar
    </a>
    @endif
</form>
...
<div class="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card)] overflow-hidden">
    ...
    <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="text-[14px] font-semibold text-[var(--text-700)]">Aún no has generado liquidaciones de cesantías</p>
        <p class="text-[12px] text-[var(--text-400)] mt-1">Genera la primera para uno de tus clientes con nóminas del año ya cargadas</p>
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-[var(--border-default)]">
                    <th class="text-[11px] font-medium text-[var(--text-400)] uppercase tracking-[0.06em] px-6 py-3 text-left">Año</th>
                    ...
                    <th class="text-[11px] font-medium text-[var(--text-400)] uppercase tracking-[0.06em] px-6 py-3 text-right">Cesantías + intereses</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-[var(--surface-muted)] hover:bg-[var(--surface-subtle)]">
                    <td class="px-6 py-[14px]">
                        <a href="{{ route('cesantia-settlements.show', $settlement, false) }}" class="font-semibold text-[14px] text-[var(--text-900)] hover:text-[var(--color-primary)]">
                            {{ $settlement->year }}
                        </a>
                    </td>
        ...
        <div class="relative bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card-hover)] w-full max-w-lg overflow-hidden"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-[var(--border-default)]">
                <div class="flex items-center gap-2.5">
                    ...
                    <h2 class="text-[16px] font-semibold text-[var(--text-900)]">Generar liquidación de cesantías</h2>
                </div>
