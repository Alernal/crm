...
<div x-data="generarPrimaModal()" @keydown.escape.window="open && cerrar()">
...
{{-- Filtros --}}
<form method="GET" action="{{ route('prima-settlements.index', [], false) }}" class="flex flex-col sm:flex-row gap-3 mb-5">
    ...
    <a href="{{ route('prima-settlements.index', [], false) }}" class="h-10 flex items-center px-4 rounded-[var(--radius-control)] border border-[var(--border-default)] text-[var(--text-700)] text-[14px] font-medium hover:bg-[var(--surface-muted)]">
        Limpiar
    </a>
    @endif
</form>
...
<div class="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card)] overflow-hidden">
    ...
    <div class="flex flex-col items-center justify-center py-16 text-center">
        <div class="w-16 h-16 rounded-[var(--radius-card)] bg-[var(--surface-muted)] flex items-center justify-center mb-4">
            <x-lucide-gift class="w-8 h-8 text-[var(--text-400)]" />
        </div>
        <p class="text-[14px] font-semibold text-[var(--text-700)]">Aún no has generado liquidaciones de prima</p>
        <p class="text-[12px] text-[var(--text-400)] mt-1">Genera la primera para uno de tus clientes con nóminas del semestre ya cargadas</p>
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-[var(--border-default)]">
                    ...
                    <th class="text-[11px] font-medium text-[var(--text-400)] uppercase tracking-[0.06em] px-6 py-3 text-left hidden sm:table-cell">Fecha de pago</th>
                    <th class="text-[11px] font-medium text-[var(--text-400)] uppercase tracking-[0.06em] px-6 py-3 text-right">Total prima</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-[var(--surface-muted)] hover:bg-[var(--surface-subtle)]">
                    <td class="px-6 py-[14px]">
                        <a href="{{ route('prima-settlements.show', $settlement, false) }}" class="font-semibold text-[14px] text-[var(--text-900)] hover:text-[var(--color-primary)]">
                            {{ \App\Models\PrimaSettlement::SEMESTERS[$settlement->semester] }} {{ $settlement->year }}
                        </a>
                    </td>
                    <td class="px-6 py-[14px] text-[14px] text-[var(--text-700)]">{{ $settlement->client->name }}</td>
                    <td class="px-6 py-[14px] hidden sm:table-cell text-[14px] text-[var(--text-700)]">{{ $settlement->payment_date->format('d/m/Y') }}</td>
                    <td class="px-6 py-[14px] text-right text-[14px] font-semibold text-[var(--text-900)]">
                        $ {{ number_format($settlement->total_prima, 0, ',', '.') }}
                    </td>
                    <td class="px-6 py-[14px] text-right">
                        <a href="{{ route('prima-settlements.show', $settlement, false) }}" class="text-[var(--text-400)] hover:text-[var(--text-900)]" title="Ver">
...
{{-- ══════════════ Modal — Generar liquidación de prima ══════════════ --}}
<div x-show="open"
    ...
    <div class="flex min-h-full items-start justify-center p-4 pt-10">
        <div class="relative bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card-hover)] w-full max-w-lg overflow-hidden"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-[var(--border-default)]">
                <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 bg-[var(--color-primary-light)] rounded-[var(--radius-control)] flex items-center justify-center">
                        <x-lucide-gift class="w-4 h-4 text-[var(--color-primary)]" />
                    </div>
                    <h2 class="text-[16px] font-semibold text-[var(--text-900)]">Generar liquidación de prima</h2>
                </div>
            ...
            <form method="POST" action="{{ route('prima-settlements.store', [], false) }}" class="px-6 py-5 space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-[13px] font-medium text-[var(--text-700)] mb-1">Semestre <span class="text-[var(--color-danger)]">*</span></label>
                        <select name="semester" x-model="semester" @change="suggestPaymentDate()" class="{{ $fieldClass }}" required>
                            @foreach(\App\Models\PrimaSettlement::SEMESTERS as $val => $lbl)
                ...
                <div>
                    ...
                    <p class="mt-1.5 text-[12px] text-[var(--text-400)]">Sugerido: 30 de junio (1er semestre) o 20 de diciembre (2do semestre).</p>
                </div>

                <p class="text-[12px] text-[var(--text-400)]">
                    Se suman las provisiones de prima ya calculadas en las nóminas generadas para este cliente dentro del semestre seleccionado.
                </p>
...
function generarPrimaModal() {
    return {
        ...
        paymentDate: '',
        suggestPaymentDate() {
            this.paymentDate = this.semester === '1' ? `${this.year}-06-30` : `${this.year}-12-20`;
        },
