...
<title>Liquidación de prima {{ $primaSettlement->client->name }}</title>
<style>
...
<div class="page-wrap">
    ...
    <div class="header clearfix">
        <div class="header-left">
            <div class="company-name">{{ $primaSettlement->client->name }}</div>
            <div class="company-detail">
                {{ $primaSettlement->client->document_type }}: {{ $primaSettlement->client->full_document }}
            </div>
        </div>
        <div class="header-right">
            <div class="doc-title">LIQUIDACIÓN DE PRIMA</div>
            <div class="doc-dates">
                <strong>Período:</strong> {{ \App\Models\PrimaSettlement::SEMESTERS[$primaSettlement->semester] }} {{ $primaSettlement->year }}<br/>
                <strong>Fecha de pago:</strong> {{ $primaSettlement->payment_date->format('d/m/Y') }}
            </div>
        </div>
    </div>
    ...
    <table class="payroll">
        <thead>
            <tr>
                <th class="name-col" style="width:50%">Empleado</th>
                <th>Días</th>
                <th>Valor prima</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td>{{ number_format($item->prima_value, 0, ',', '.') }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr>
                ...
                <td>{{ number_format($primaSettlement->items->sum('worked_days'), 0, ',', '.') }}</td>
                <td>{{ number_format($primaSettlement->total_prima, 0, ',', '.') }}</td>
            </tr>
        </tfoot>
    </table>
