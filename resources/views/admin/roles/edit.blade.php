...
<div class="max-w-3xl">
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="divide-y divide-gray-200">
            @csrf @method('PATCH')

            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-2">Nombre <span class="text-red-400">*</span></label>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach ($modules as $mod)
                                @php $perm = $permissions->get($mod->key); @endphp
                                <tr class="hover:bg-gray-50/50" x-data="permRow(
                                    {{ $perm?->can_view   ? 'true':'false' }},
                                    {{ $perm?->can_create ? 'true':'false' }},
                                    {{ $perm?->can_edit   ? 'true':'false' }},
                                    {{ $perm?->can_delete ? 'true':'false' }})">
                                    <td class="px-4 py-3 font-medium text-gray-700">{{ $mod->name }}</td>
                                    @foreach ([['view','can_view'],['create','can_create'],['edit','can_edit'],['delete','can_delete']] as [$action,$col])
                                        <td class="px-3 py-3 text-center">
                                            <input type="checkbox"
                                                   name="permissions[{{ $mod->key }}][{{ $action }}]"
                                                   value="1" x-model="perms.{{ $action }}"
                                                   class="w-4 h-4 rounded border-gray-300 text-blue-700 focus:ring-blue-400/40">
                                        </td>
                                    @endforeach
                                    <td class="px-3 py-3 text-center">
...
<script>
function permRow(view, create, edit, del) {
    return {
        perms: { view, create, edit, delete: del },
