...
<div class="flex items-start justify-between mb-6 gap-4">
    <div>
        <a href="{{ route('cartera.client', $invoice->client) }}"
        ...
        <div class="flex items-center gap-3 flex-wrap">
            <p class="text-[22px] font-bold text-[var(--text-900)] font-mono tracking-tight">{{ $invoice->number }}</p>
...
<div class="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card)] px-6 py-5">
    <div class="flex items-center justify-between mb-3">
        <h2 class="text-[16px] font-bold text-[var(--text-900)]">Progreso de cobro</h2>
...
<div class="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card)] overflow-hidden">
    ...
    <table class="w-full">
        <thead>
            <tr>
                @php
                    $thClassPay = 'bg-[var(--surface-card)] border-b border-[var(--border-default)] text-[13px] font-bold text-[var(--text-900)] uppercase tracking-[0.06em] px-6 py-3.5';
                @endphp
                <th class="{{ $thClassPay }} text-left">Fecha</th>
                <th class="{{ $thClassPay }} text-right">Monto</th>
                <th class="{{ $thClassPay }} text-left hidden sm:table-cell">Método</th>
                <th class="{{ $thClassPay }} text-left hidden md:table-cell">Referencia</th>
                <th class="{{ $thClassPay }} text-left hidden lg:table-cell">Notas</th>
                <th class="{{ $thClassPay }} text-center hidden lg:table-cell">Soporte</th>
                <th class="{{ $thClassPay }} w-12"></th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr class="border-b border-[var(--surface-muted)] border-l-[3px] border-l-transparent hover:border-l-[var(--color-primary)] hover:bg-[var(--surface-subtle)] group">
                <td class="px-6 py-[14px]">
                    <p class="text-[14px] text-[var(--text-700)]">{{ $payment->payment_date->format('d/m/Y') }}</p>
                    <p class="text-[13px] text-[var(--text-400)] mt-0.5 sm:hidden">{{ $methodLabels[$payment->payment_method] ?? $payment->payment_method }}</p>
                </td>
                <td class="px-6 py-[14px] text-right">
                    <p class="text-[14px] text-[var(--color-success)] tabular-nums">$ {{ number_format($payment->amount, 0, ',', '.') }}</p>
                </td>
                ...
                <td class="px-6 py-[14px] hidden md:table-cell">
                    <span class="font-mono text-[14px] text-[var(--text-500)]">{{ $payment->reference ?: '—' }}</span>
                </td>
                <td class="px-6 py-[14px] hidden lg:table-cell">
                    <span class="text-[14px] text-[var(--text-500)]">{{ $payment->notes ?: '—' }}</span>
                </td>
                ...
                <td class="px-6 py-[14px] text-right">
                    ...
                    <form method="POST" action="{{ route('payments.destroy', $payment) }}"
                        ...
                        <button type="submit"
                                title="Eliminar pago"
                                class="p-1.5 rounded-[var(--radius-control)] hover:bg-[var(--surface-muted)] text-[var(--text-400)] hover:text-[var(--text-900)] opacity-0 group-hover:opacity-100">
        ...
        <tfoot>
            <tr class="bg-[var(--surface-card)] border-t-2 border-t-[var(--border-default)]">
                <td class="px-6 py-3 text-[13px] font-semibold text-[var(--text-900)] uppercase tracking-[0.06em]">Total recibido</td>
    ...
    <div class="px-6 py-10 text-center">
        ...
        <p class="text-[15px] font-semibold text-[var(--text-500)]">Sin pagos registrados</p>
        @if($canPay)
        <button @click="modalPago = true"
                class="mt-3 inline-flex items-center gap-1.5 h-10 px-5 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white rounded-[var(--radius-control)] text-[15px] font-medium">
...
<div x-data="{ open: false }"
     class="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card)] overflow-hidden">

    <button @click="open = !open"
            class="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-[var(--surface-subtle)]">
        <h2 class="text-[16px] font-bold text-[var(--text-900)]">
    ...
    <div x-show="open"
        ...
        <table class="w-full">
            <thead>
                <tr>
                    @php
                        $thClassItems = 'bg-[var(--surface-card)] border-b border-[var(--border-default)] text-[13px] font-bold text-[var(--text-900)] uppercase tracking-[0.06em] px-6 py-3.5';
                    @endphp
                    <th class="{{ $thClassItems }} text-left">Descripción</th>
                    <th class="{{ $thClassItems }} text-right hidden sm:table-cell">Cantidad</th>
                    <th class="{{ $thClassItems }} text-right hidden sm:table-cell">Precio unit.</th>
                    <th class="{{ $thClassItems }} text-right hidden sm:table-cell">IVA</th>
                    <th class="{{ $thClassItems }} text-right">Subtotal</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="border-b border-[var(--surface-muted)] border-l-[3px] border-l-transparent hover:border-l-[var(--color-primary)] hover:bg-[var(--surface-subtle)]">
                    <td class="px-6 py-[14px]">
                        <p class="text-[14px] text-[var(--text-900)]">{{ $item->description }}</p>
                        @if($item->vat_rate > 0)
                        <p class="text-[13px] text-[var(--color-warning)] mt-0.5 sm:hidden">IVA {{ number_format($item->vat_rate, 0) }}%</p>
                        @endif
                    </td>
                    <td class="px-6 py-[14px] text-right text-[14px] text-[var(--text-700)] hidden sm:table-cell tabular-nums">
                    ...
                    <td class="px-6 py-[14px] text-right hidden sm:table-cell">
                        @if($item->vat_rate > 0)
                        <span class="text-[13px] text-[var(--color-warning)] font-medium">{{ number_format($item->vat_rate, 0) }}%</span>
                        @else
                        <span class="text-[var(--border-strong)]">—</span>
                        @endif
                    </td>
                    <td class="px-6 py-[14px] text-right text-[14px] text-[var(--text-900)] tabular-nums">
...
{{-- ── Cliente (barra lateral) ──────────────────────────────── --}}
<div class="lg:col-span-1">
<div class="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] shadow-[var(--shadow-card)] px-6 py-5">
    <h2 class="text-[16px] font-bold text-[var(--text-900)] mb-4">Datos del cliente</h2>
    <div class="flex items-center gap-3 mb-5">
        ...
        <p class="text-[14px] font-semibold text-[var(--text-900)] leading-tight">{{ $invoice->client->name }}</p>
    </div>
    <div class="space-y-4">
        <div>
        ...
        <div>
            ...
            <p class="text-[14px] text-[var(--text-700)] break-all">{{ $invoice->client->email }}</p>
        </div>
        @endif
