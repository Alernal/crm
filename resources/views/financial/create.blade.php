<x-app-layout>
<x-slot name="title">{{ $group === 'estados_financieros' ? 'Nuevo Estado Financiero' : 'Nuevo Presupuesto' }}</x-slot>
...
@php
    ...
    $backIndexRoute  = $group === 'estados_financieros' ? 'financial.statements.index'  : 'financial.index';
    $backClientRoute = $group === 'estados_financieros' ? 'financial.statements.client' : 'financial.client';
@endphp

<a href="{{ $preClient ? route($backClientRoute, $preClient) : route($backIndexRoute) }}"
...
{{-- Botón flotante de guardar — siempre visible sin importar el scroll,
     mismo criterio que `financial/statements/create.blade.php` y
     `financial/edit.blade.php` (a pedido explícito del usuario). --}}
<button type="submit"
        ...
        title="{{ $group === 'estados_financieros' ? 'Guardar estado financiero' : 'Guardar presupuesto' }}">
...
<div class="bg-[var(--surface-card)] rounded-[var(--radius-card)] border border-[var(--border-default)] shadow-[var(--shadow-card)] overflow-hidden mb-4">
    ...
    <div class="px-6 pt-5 pb-4">
        ...
        {{-- Fila 1: Cliente · Tipo · Período base · Período / N° --}}
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div>
                ...
                @if($group === 'presupuestos')
                <input type="hidden" name="type" value="flujo_caja">
                <div class="{{ $inputClass }} flex items-center gap-2 bg-[var(--surface-muted)] text-[var(--text-700)]">
            ...
            {{-- Presupuestos: Período base fijo (proyecta hacia adelante) --}}
            <template x-if="!isStatementType">
                <div>
                    <label class="{{ $labelClass }}">
            ...
            {{-- Estados Financieros: el año de cada período se define abajo, en "Períodos a registrar" --}}
            <template x-if="isStatementType">
                <input type="hidden" name="base_year" :value="periodYears[0] || {{ date('Y') }}"/>
            </template>

            <div class="flex gap-3" :class="(isStatementType) ? 'col-span-2 lg:col-span-2' : ''">
                <div class="flex-1">
                    <label class="{{ $labelClass }}">
                        Período <span class="text-[var(--color-danger)]">*</span>
                    </label>
                    <select name="period_type" x-model="periodType" @change="clampPeriodsCount()" required class="{{ $inputClass }}">
                        @foreach($periodTypes as $val => $label)
                        <option value="{{ $val }}" {{ old('period_type', 'annual') === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <template x-if="!isStatementType && periodType === 'monthly'">
                    <div class="w-32">
                        ...
                        <select name="base_month" x-model.number="baseMonth" @change="clampPeriodsCount()" class="{{ $inputClass }}">
                            ...
                            <option value="{{ $i + 1 }}" {{ (int) old('base_month', 1) === $i + 1 ? 'selected' : '' }}>{{ $mes }}</option>
                            @endforeach
                        </select>
                    </div>
                </template>
            ...
            <template x-if="!isStatementType && periodType !== 'annual'">
        ...
        {{-- Fila 2: Nombre · Notas (Presupuestos) — Estados Financieros se nombran solos --}}
        <template x-if="!isStatementType">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 pb-4 border-t border-[var(--border-default)] pt-4">
                <div>
                    <label class="{{ $labelClass }}">
        ...
        {{-- Fila 3c: Flujo de caja — supuestos sugeridos (inflación/SMMLV) --}}
        <template x-if="selectedType === 'flujo_caja'">
            <div class="pt-4 border-t border-[var(--border-default)]">
                @if($preClient)
                    @if(!$suggestedVariables || ($suggestedVariables['inflation'] == 0 && $suggestedVariables['smmlv_increase'] == 0))
                    <div class="flex items-center gap-2 bg-[var(--color-warning-bg)] border border-[#FCD34D] rounded-[var(--radius-control)] px-4 py-2.5 text-[12px] text-[var(--color-warning-text)]">
                        <x-lucide-alert-triangle class="w-3.5 h-3.5 flex-shrink-0" />
                        {{ $preClient->name }} no tiene inflación ni SMMLV configurados en
                        <a href="{{ route('financial.data', $preClient) }}" class="underline font-medium">Datos</a>.
                        Los rubros con proyección automática (inflación/SMMLV) partirán de una tasa de 0% hasta que los configures.
                    </div>
                    @else
                    <div class="flex items-start gap-2.5 bg-[var(--color-primary-light)] border border-[var(--color-primary)]/20 rounded-[var(--radius-control)] px-4 py-2.5 text-[12px] text-[var(--color-primary-dark)]">
                        <x-lucide-sparkles class="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                        <span>
                            <strong>Supuestos sugeridos de {{ $preClient->name }}:</strong>
                            inflación {{ number_format($suggestedVariables['inflation'], 2) }}% ·
                            SMMLV +{{ number_format($suggestedVariables['smmlv_increase'], 2) }}%.
                            Se aplican automáticamente a los rubros con driver "Inflación"/"SMMLV" al crear el presupuesto — edítalos en
                            <a href="{{ route('financial.data', $preClient) }}" class="underline font-medium">Datos</a> si no aplican a este cliente.
                        </span>
                    </div>
                    @endif
                @else
                <p class="text-[12px] text-[var(--text-400)]">Selecciona un cliente arriba para ver sus supuestos sugeridos (inflación, SMMLV) de Datos.</p>
                @endif
            </div>
        </template>

        {{-- Fila 3d: ESF/ERI — vínculo con la contraparte --}}
        <template x-if="selectedType === 'esf'">
            <div class="pt-4 border-t border-[var(--border-default)]">
        ...
        <template x-if="selectedType === 'eri'">
            <div class="pt-4 border-t border-[var(--border-default)]">
...
<div class="mb-5">
    ...
    <div class="flex items-center justify-between mb-3">
        <p class="text-[11px] font-medium text-[var(--text-400)] uppercase tracking-[0.06em]">
            {{ $group === 'estados_financieros' ? 'Estructura del estado financiero (rubros NIIF)' : 'Estructura del presupuesto' }}
        </p>
        <template x-if="!isStatementType">
    ...
    <template x-if="isStatementType">
        <p class="text-[11px] text-[var(--text-400)] -mt-2 mb-3">
    ...
    <template x-if="sections.length === 0">
        <div class="flex flex-col items-center justify-center py-14 bg-[var(--surface-card)] border border-dashed border-[var(--border-strong)] rounded-[var(--radius-card)] text-center">
            ...
            <p class="text-[14px] font-semibold text-[var(--text-500)]">{{ $group === 'estados_financieros' ? 'Selecciona el tipo de estado financiero arriba' : 'Selecciona el tipo de presupuesto arriba' }}</p>
            <p class="text-[12px] text-[var(--text-400)] mt-1">La plantilla se carga automáticamente con todos los rubros{{ $group === 'estados_financieros' ? ' bajo NIIF' : '' }}</p>
        </div>
    </template>
        ...
        <div class="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[var(--radius-card)] overflow-hidden mb-3 shadow-[var(--shadow-card)]">
            ...
            <template x-for="(line, lIdx) in section.lines" :key="lIdx">
                <div class="flex flex-wrap items-center gap-2 px-5 py-2.5 border-b border-[var(--surface-muted)] hover:bg-[var(--surface-subtle)] group transition-opacity"
                    ...
                    {{-- Variable de proyección (no aplica a Estados Financieros: cifras reales digitadas por período) --}}
                    <template x-if="!isStatementType">
                    ...
                    {{-- Estados Financieros: cuadro de valor por período (sin driver de proyección) --}}
                    <template x-if="isStatementType">
                        <div class="flex items-center gap-1 overflow-x-auto max-w-full py-0.5">
                            <template x-for="p in periodsArray()" :key="p">
                                <div class="flex-shrink-0 w-[88px]">
                                    <label class="block text-[9px] text-[var(--text-400)] mb-0.5" x-text="p === 0 ? 'Base' : 'Período ' + p"></label>
                                    <input type="number" step="0.01"
                                           x-model="line.values[p]"
                                           :name="`sections[${sIdx}][lines][${lIdx}][values][${p}]`"
                                           ...
                                           :class="isAutoCell(line.name, p)
                                                ...
                                                : 'bg-[var(--surface-card)] text-[var(--text-900)]'"
                                           class="w-full border border-[var(--border-default)] rounded-[var(--radius-control)] px-2 py-1.5 text-[12px] text-right outline-none tabular-nums focus:ring-1 focus:ring-[var(--color-primary-light)]"/>
                                </div>
                            </template>
                        </div>
                    </template>
...
function budgetForm() {
    ...
    return {
        selectedType: '{{ $group === "presupuestos" ? "flujo_caja" : old("type", "") }}',
        // Fija desde el servidor según el submenú (Presupuestos vs Estados
        // Financieros) — a diferencia de `selectedType`, no depende de que el
        // usuario ya haya elegido ESF/ERI en el <select>, así que evita que
        // los campos de Presupuestos se muestren un instante antes de elegir.
        isStatementType: {{ $group === 'estados_financieros' ? 'true' : 'false' }},
        periodType: '{{ old('period_type', 'annual') }}',
        periodsCount: {{ (int) old('periods_count', 3) }},
        baseMonth: {{ (int) old('base_month', 1) }},
        ...
        loadDefaults() {
            if (this.selectedType === 'flujo_caja') {
                this.periodType = 'monthly';
            }
            if (this.selectedType === 'esf' || this.selectedType === 'eri') {
        ...
        addLine(sIdx) {
            this.sections[sIdx].lines.push({
                name: '', driver: 'manual', signNegative: false,
                customRate: '', baseValue: 0, displayValue: '',
