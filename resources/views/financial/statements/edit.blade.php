<x-app-layout>
<x-slot name="title">Editar Estado Financiero</x-slot>
...
@php
    ...
    $currentSections = fn (\App\Models\Budget $b) => $b->sections->map(fn ($s) => [
        'name'           => $s->name,
        'statement_role' => $s->statement_role,
        'lines' => $s->lines->map(fn ($l) => [
            'name'         => $l->name,
            'signNegative' => $l->sign_negative,
            'values'       => collect(range(0, $b->periods_count))->mapWithKeys(fn ($i) => [$i => $l->getValueForPeriod($i)])->all(),
        ])->values()->all(),
    ])->values()->all();
@endphp
...
<form method="POST" action="{{ route('financial.statements.update', $esf) }}"
      x-data="statementPairEditForm()"
      x-init="init()">
@csrf @method('PUT')
...
{{-- Botón flotante de guardar — siempre visible sin importar el scroll,
     ...
     quiere guardar un cambio (a pedido explícito del usuario). Es un botón
     submit normal dentro del propio <form>, solo posicionado `fixed`. --}}
<button type="submit"
...
<div class="mb-6">
    <button type="button" @click="parametrizacionModal = true"
            class="inline-flex items-center gap-1.5 h-9 px-3.5 rounded-[var(--radius-control)] bg-[var(--surface-subtle)] border border-[var(--border-default)] text-[13px] font-medium text-[var(--text-700)] hover:bg-[var(--surface-muted)]">
            ...
            <div class="flex flex-col lg:flex-row gap-4">
                <div class="lg:flex-1">
                    ...
                    <select name="period_type" x-model="periodType" @change="onPeriodTypeChange()" required class="{{ $inputClass }} w-full">
                        ...
                        <option value="{{ $val }}" {{ old('period_type', $esf->period_type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
...
<script>
function statementPairEditForm() {
    const raw = { esf: {!! json_encode($currentSections($esf)) !!}, eri: {!! json_encode($currentSections($eri)) !!} };

    return {
        activeTab: '{{ $activeTab }}',
        parametrizacionModal: false,
        periodType: '{{ old('period_type', $esf->period_type) }}',
        periodsCount: {{ (int) old('periods_count', $esf->periods_count) }},
        periodYears: @json(old('period_years', $periodYears)),
        periodLabels: @json(old('period_labels', $periodLabels)),
        cutoffDate: '{{ old('cutoff_date', $esf->cutoff_date?->format('Y-m-d')) }}',
