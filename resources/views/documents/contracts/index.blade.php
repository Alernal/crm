<x-app-layout>
<x-slot name="title">Contratos y Propuestas</x-slot>

@php
    $statusLabels = \App\Models\GeneratedDocument::STATUSES;

    // Cada tipo de documento generado (contrato/propuesta) tiene su propio juego de
    // rutas 'documents.contracts.*'/'documents.proposals.*' — este índice es compartido
    // entre ambos (lista todo GeneratedDocument sin filtrar por tipo), así que resuelve
    // el nombre de ruta según documentType->key en vez de asumir uno solo.
    $routesByType = [
        'contrato_servicios' => ['show' => 'documents.contracts.show', 'pdf' => 'documents.contracts.pdf', 'destroy' => 'documents.contracts.destroy'],
        'propuesta_comercial' => ['show' => 'documents.proposals.show', 'pdf' => 'documents.proposals.pdf', 'destroy' => 'documents.proposals.destroy'],
    ];
@endphp

<div class="flex items-start justify-between mb-6 gap-4">
    <div>
        <p class="text-[22px] font-bold text-[var(--text-900)]">Contratos y Propuestas</p>
        <p class="text-[13px] text-[var(--text-500)] mt-0.5">Motor Documental — genera y administra contratos y propuestas a partir de plantillas.</p>
    </div>
    <div class="flex items-center gap-2 flex-shrink-0">
        <a href="{{ route('documents.proposals.wizard') }}"
           class="inline-flex items-center gap-[6px] h-10 px-5 rounded-[var(--radius-control)] bg-[var(--surface-subtle)] border border-[var(--border-default)] hover:bg-[var(--surface-muted)] text-[var(--text-700)] text-[14px] font-medium">
            <x-lucide-plus class="w-4 h-4" />
            Nueva Propuesta
        </a>
        <a href="{{ route('documents.contracts.wizard') }}"
...
{{-- Filtros --}}
<form method="GET" action="{{ route('documents.contracts.index') }}" class="mb-5">
    <div class="flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        <select name="year" class="h-10 border border-[var(--border-default)] rounded-[var(--radius-control)] px-3.5 text-[14px] bg-[var(--surface-subtle)] text-[var(--text-700)] focus:ring-2 focus:ring-[var(--color-primary-light)] focus:border-[var(--color-primary)] outline-none min-w-[110px]">
        ...
        @if(request()->hasAny(['q','status','client_id','year']))
        <a href="{{ route('documents.contracts.index') }}" class="h-10 flex items-center px-4 rounded-[var(--radius-control)] bg-[var(--surface-subtle)] border border-[var(--border-default)] text-[var(--text-700)] text-[14px] font-medium hover:bg-[var(--surface-muted)]">
            Limpiar
        </a>
        @endif
    </div>
...
<div class="text-center py-16">
    <div class="w-14 h-14 rounded-[var(--radius-card)] bg-[var(--color-primary-light)] flex items-center justify-center mx-auto mb-4">
        <x-lucide-file-signature class="w-7 h-7 text-[var(--color-primary)]" />
    </div>
    @if(request()->hasAny(['q','status','client_id','year']))
    <p class="text-[14px] font-semibold text-[var(--text-700)]">No se encontraron documentos con estos filtros</p>
    <a href="{{ route('documents.contracts.index') }}" class="mt-3 inline-block text-[13px] text-[var(--color-primary)] font-medium">Limpiar filtros</a>
    @else
    <p class="text-[14px] font-semibold text-[var(--text-700)]">Aún no has generado ningún contrato ni propuesta</p>
    <p class="text-[12px] text-[var(--text-400)] mt-1">Genera el primero seleccionando un cliente y una plantilla</p>
    <div class="mt-4 flex items-center justify-center gap-2">
        <a href="{{ route('documents.proposals.wizard') }}"
           class="inline-flex items-center gap-[6px] h-10 px-5 bg-[var(--surface-subtle)] border border-[var(--border-default)] hover:bg-[var(--surface-muted)] text-[var(--text-700)] text-[14px] font-medium rounded-[var(--radius-control)]">
            <x-lucide-plus class="w-4 h-4" />
            Nueva Propuesta
        </a>
        <a href="{{ route('documents.contracts.wizard') }}"
           class="inline-flex items-center gap-[6px] h-10 px-5 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white text-[14px] font-medium rounded-[var(--radius-control)]">
            <x-lucide-plus class="w-4 h-4" />
            Nuevo Contrato
        </a>
    </div>
...
<div class="overflow-y-auto max-h-[65vh]">
    <table class="w-full">
        ...
        <tbody>
            ...
            @php
                $st = $statusLabels[$document->status] ?? ['label' => $document->status, 'variant' => 'neutral'];
                $typeRoutes = $routesByType[$document->documentType->key] ?? $routesByType['contrato_servicios'];
            @endphp
            <tr class="border-b border-[var(--surface-muted)] border-l-[3px] border-l-transparent hover:border-l-[var(--color-primary)] hover:bg-[var(--surface-subtle)]">
                <td class="px-6 py-[14px]">
                    <a href="{{ route($typeRoutes['show'], $document) }}"
                ...
                <td class="px-6 py-[14px] hidden md:table-cell text-[14px] text-[var(--text-500)]">{{ $document->documentType->label }}</td>
                ...
                <td class="px-6 py-[14px] text-right">
                    <div class="flex items-center justify-end gap-[10px]">
                        ...
                        <a href="{{ route($typeRoutes['pdf'], $document) }}"
                           class="text-[var(--text-400)] hover:text-[var(--text-900)]" title="Descargar PDF">
                            <x-lucide-download class="w-4 h-4" />
                        </a>
                        <form method="POST" action="{{ route($typeRoutes['destroy'], $document) }}"
                              x-data=""
